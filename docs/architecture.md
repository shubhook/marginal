...
- A drag that starts *inside* the strip (near the boundary) is captured here regardless of which side it ends up staying on or crossing into — buffered as a flat list of `{x, y}` screen points (`pointerdown` seeds the list, `pointermove` appends, pointer capture via `setPointerCapture` keeps delivering events once the cursor leaves the strip's own bounds so a fast drag doesn't lose points).
